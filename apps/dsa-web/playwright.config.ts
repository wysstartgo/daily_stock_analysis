...
function resolveBackendCommand() {
  ...
  if (fs.existsSync(unixVenvPython)) {
    return `${unixVenvPython} main.py --webui-only --host 127.0.0.1 --port 8188`;
  }

  const windowsVenvPython = path.join(repoRoot, '.venv', 'Scripts', 'python.exe');
  if (fs.existsSync(windowsVenvPython)) {
    return `"${windowsVenvPython}" main.py --webui-only --host 127.0.0.1 --port 8188`;
  }

  return 'python main.py --webui-only --host 127.0.0.1 --port 8188';
}
  ...
  webServer: [
    {
      ...
      cwd: repoRoot,
      url: 'http://127.0.0.1:8188/api/v1/auth/status',
